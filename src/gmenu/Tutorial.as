package gmenu 
{
	 ...
	 * for original version of this class.
	 * 
	...
	public class Tutorial extends Screens 
	{
		...
		override public function begin():void 
		{
			...
			instructions.push(new GameInstructions(Assets.TUTORIAL1_BACKGROUND));
			instructions.push(new GameInstructions(Assets.TUTORIAL2_BACKGROUND));
			instructions.push(new GameInstructions(Assets.TUTORIAL3_BACKGROUND));
			instructions.push(new GameInstructions(Assets.TUTORIAL4_BACKGROUND));
			
			super.begin();
			
			// navigation buttons creation
			add(new NavigationButton(30, 500, "< Prev", false, 80, 40, changePage, currentPage - 1));
			
			var navLength:int = 40 * instructions.length + 10 * (instructions.length - 1);
			
			for (var i:int = 0; i < instructions.length; ++i)
			{
				add(new NavigationButton((FP.width - navLength) * 0.5 + 50 * i, 500, (i + 1).toString(), currentPage == i, 40, 40, changePage, i));
			}
			
			add(new NavigationButton(690, 500, "Next >", false, 80, 40, changePage, currentPage + 1));
			
			menu = new NavigationButton(360, 550, "Menu", false, 80, 40, menuButtonClick)
			add(menu);
